...
class Network(nn.Module):
    def __init__(self, input_features=1 * 28 * 28):
        super().__init__()
...
batch_size = 256
train_loader, test_loader = mnist(root='./../data/', batch_size=batch_size)
...
network = Network()
network.to(device)
# network.load_state_dict(torch.load('./../weights/mnist-linear.h5'))
...
for epoch in range(1, epochs + 1):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        x_true_batch = x_true_batch.view(x_true_batch.size()[0], 1 * 28 * 28)
...
torch.save(network.state_dict(), './../weights/mnist-linear.h5')
...
for i, data in enumerate(test_loader, 0):
    ...
    x_test_batch = x_test_batch.view(x_test_batch.size()[0], 1 * 28 * 28)
