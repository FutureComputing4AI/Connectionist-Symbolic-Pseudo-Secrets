...
embedding = np.reshape(x_pred, (x_pred.shape[0], 3 * 84 * 84))
...
kmeans = cluster.KMeans(n_clusters=100, random_state=0).fit(embedding)
...
spectral = cluster.SpectralClustering(n_clusters=100, random_state=0, affinity='nearest_neighbors').fit(embedding)
...
gm = mixture.GaussianMixture(n_components=100, random_state=0, covariance_type='diag').fit(embedding)
...
birch = cluster.Birch(n_clusters=100).fit(embedding)
